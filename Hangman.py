...
def play(word):
    ...
    for c in word_completion:
        print(c, end=' ')
    while True:
        print("Хотите ввести букву или все слово целиком?")
        choise = input()
        if choise.lower() == "слово":
            answer = input("Введите ваше слово: ")
            if not answer in guessed_words:
            ...
            while not letter.isalpha() and len(letter) != 1:
                letter = input("Может букву введем все-таки? ")
            ...
            for c in word:
                if letter.upper() == c:
                    ...
                    if index == 0:
                        word_completion = letter + word_completion[1:]
                    else:    
                        word_completion = word_completion[:index] + letter + word_completion[index + 1:]
                    count += 1
            if count == 0:
                print("Увы, такой буквы тут нет")
            elif count == 1:
                print("Буква", letter, "есть в слове!")
            else:
                print("Буква", letter, "встречается в слове", count, "раз")
            for c in word_completion:
                print(c, end=' ')            
            display_hangman(tries)
